...
pub async fn get_saved_response(
    ...
    // To deal with nullable columns, ask sqlx to forcefully assume that the columns will not be null
    // if incorrect, will cause a runtime error.
...
pub async fn try_processing(
    ...
    // NOTE(aalhendi): Use a transaction to enable concurrent request idempotency.
    // Because READ COMMITTED is the default isolation level in Postgres, INSERT statement
    // if fired by 2nd request must wait for outcome of the SQL transaction started by first request.
    // This will NOT work when using stricter isolation levels such as `repeatable read` or `serializable`
