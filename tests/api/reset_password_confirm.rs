...
async fn reset_password_confirm_succeeds_for_valid_token() {
    ...
    let reset_get_response = app.get_reset_password_confirm(&token).await;
    assert_eq!(200, reset_get_response.status().as_u16());

    // Act
    let confirm_body = serde_json::json!({"token": token.as_ref(),"password": &password,"new_password_check": &password});
...
async fn clicking_on_password_reset_confirmation_link_returns_200() {
    // Arrange
    let app = spawn_app().await;
    let body = app.test_user.email.clone();
